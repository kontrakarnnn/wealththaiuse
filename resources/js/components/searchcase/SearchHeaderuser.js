...
export default class SearchHeaderuser extends Component {
      ...
      return <div className="card-body" style={{overflowX:'auto'}}>
        <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
          ...
          {this.state.casefilter.map(data =>
            <tr role="row" className="table-tr" data-url={'/wealththaiinsurance/cases/'+data.id+'/detail/showuser'}>
            <td>{data.id}</td>
...
if (document.getElementById('searchheaderuser')) {
  const component = document.getElementById('searchheaderuser');
  const props = Object.assign({}, component.dataset);
    ReactDOM.render(<SearchHeaderuser {...props}/>, component);
}
