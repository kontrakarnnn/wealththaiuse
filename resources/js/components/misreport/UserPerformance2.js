...
export default class UserPerformance extends Component {
  ...
  componentDidMount() {
    //console.log("rowchart"+this.state.rowchart);
      ...
      axios.get('/wealththaiinsurance/report/get/alluser').then(response=>{
    ...
    if(this.state.tableflag == 1)
    {
      ...
      if(this.state.typeofdate == 1)
      {
        ...
        for (var i=0; i <= diff.days(); i++)
        {
          ...
          console.log("dayyyyy"+i+" "+d)
          var date = format(new Date(addSubtractDate.add(d, i, "days")),'dd MMM yyyy')
          console.log("ddddddd"+i+" "+dd)
          console.log("dd.getMonth()"+i+" "+(dd.getMonth()+1))

          console.log("Day",dd.getDay(),"Month",dd.getMonth(),"year",dd.getFullYear())

          show = [dd.getDay()+'/'+(dd.getMonth()+1)+'/'+dd.getFullYear()];
