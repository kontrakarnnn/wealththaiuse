...
@section('action-content')
  ...
  <div class="box-header">
    <div class="row">
        ...
        <div class="col-sm-4">
          <a class="btn btn-primary" href="{{ route('branch.create') }}">Add new branch</a>
        </div>
    </div>
  </div>
  <!-- /.box-header -->
          ...
          <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
            ...
            @foreach ($branchs as $branch)
                <tr role="row" class="odd">
                  ...
                  <td>{{ $branch->name }}</td>
                  <td>{{ $branch->add_no }} {{ $branch->add_alley }} {{ $branch->add_road }} {{ $branch->add_subdistrict }} {{ $branch->add_district }} {{ $branch->add_city }} {{ $branch->add_country }} {{ $branch->add_postcode}}</td>
                  <td>{{ $branch->number }}</td>
                  <td>{{ $branch->tel }}</td>
                  <td>{{ $branch->fax }}</td>
        	  <td> {{ $branch->con_name }} <br> {{ $branch->con_tel }} <br> <a href="mailto:{{$branch->con_email}}" target="_top">{{ $branch->con_email}}</a> </td>
                  <td>
                    <form class="row" method="POST" action="{{ route('branch.destroy', ['id' => $branch->id]) }}" onsubmit = "return confirm('Are you sure?')">
                        <input type="hidden" name="_method" value="DELETE">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <a href="{{ route('branch.edit', ['id' => $branch->id]) }}" class="btn btn-warning  btn-margin">
							Update
                        </a>
