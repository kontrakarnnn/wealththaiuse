...
@section('action-content')
  ...
  <div class="box-header">
    <div class="row">
        <div class="col-sm-8">
          <h3 class="box-title">List of view partner</h3>
        </div>
        <div class="col-sm-4">
          <a class="btn btn-primary" href="{{ route('viewpartner.create') }}">Add new view</a>
        </div>
    </div>
  </div>
  <!-- /.box-header -->
  <div class="box-body">
      ...
      <form method="POST" action="{{ route('viewpartner.search') }}">
         {{ csrf_field() }}
                ...
                <tr role="row" class="odd">
                  ...
                  <td>{{ $view->view_url }}</td>
					...
					<td>{{ $view->add_to_side }}</td>
					        <td>{{ $view->priority }}</td>
                  <td>
                    <form class="row" method="POST" action="{{ route('viewpartner.destroy', ['id' => $view->id]) }}" onsubmit = "return confirm('Are you sure?')">
                        <input type="hidden" name="_method" value="DELETE">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <a href="{{ route('viewpartner.edit', ['id' => $view->id]) }}" class="btn btn-warning col-sm-3 col-xs-5 btn-margin">
                        Update
                        </a>
