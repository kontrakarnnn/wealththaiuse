...
<form class="form-horizontal" role="form" method="POST" action="{{ route('toolordermember.store') }}">
    {{ csrf_field() }}
<div class="container">
    <div class="row">
            ...
            <div class="panel panel-default">
              <div class="panel-heading">Confirm Your Order</div>
                ...
                <table class="table table-borderless table-hover" style="width:100%;color:black; ">
                  @foreach($data as $da)
                  <input type="hidden" type="text" class="form-control" name="tool_set_id"  value="{{$da->id}}">
                <tr>
                <th width="50%"><p>Product</p></th>
                <td >{{$da->Tool->name}}</td>

                </tr>
                <tr>
                <th width="50%"><p>Set</p></th>
                <td >{{$da->name}}</td>
