...
@section('action-content')
  ...
  <div class="box-header">
    <div class="row">
        ...
        <div class="col-sm-4">
          <a class="btn btn-primary" href="{{ route('eventmarketing.create') }}">Add new event</a>
        </div>
    </div>
  </div>
  <!-- /.box-header -->
    ...
    <div id="example2_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
      <div class="row">
          ...
          <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
            <thead>
              <tr role="row">
                ...
                <th width="" class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="event: activate to sort column ascending">Link For SmartLink</th>
            ...
            @foreach ($events as $event)
                <tr role="row" class="odd">
                  ...
                  <td> <a target="_blank" href="{{ url('/eventmember/eventregis',$event->id)  . '/event' }}">{{ url('/eventmember/eventregis',$event->id)  . '/event' }}</a>
       ...
       <td>  <a target="_blank" href="{{ url('/perregis/create') . '?refevent?' . $event->id }}">{{ url('/perregis/create') . '?refevent?' . $event->id }}</a>
        ...
        <td>{{ $event->captcha }}</td>
                  <td>
                    <form class="row" method="POST" action="{{ route('eventmarketing.destroy', ['id' => $event->id]) }}" onsubmit = "return confirm('Are you sure?')">
                        <input type="hidden" name="_method" value="DELETE">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <a href="{{ URL::to('/eventmarketing/show',$event->id)}}" class="btn btn-info  btn-margin">
                            Details
                          </a>
                          <a href="{{ URL::to('/eventmarketing',$event->id)}}" class="btn btn-primary  btn-margin">
    Member In This Event
              </a>
                        <a href="{{ route('eventmarketing.edit', ['id' => $event->id]) }}" class="btn btn-warning  btn-margin">
							Update
                        </a>
