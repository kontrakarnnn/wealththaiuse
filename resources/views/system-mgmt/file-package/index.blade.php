...
@section('action-content')
  ...
  <div class="box-header">
    <div class="row">
        ...
        <div class="col-sm-8">
          <h3 class="box-title">List of File Package</h3>
        </div>
        <div class="col-sm-4">
          <a class="btn btn-primary" href="{{ route('file-package.create') }}">Add New File Package</a>
        </div>
    </div>
  </div>
  <!-- /.box-header -->
  <div class="box-body">
      ...
      <form method="POST" action="{{ route('file-package.search') }}">
         {{ csrf_field() }}
          ...
          <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
            ...
            @foreach ($events as $event)
                <tr role="row" class="odd">
                  ...
                  <td>
                    <form class="row" method="POST" action="{{ route('file-package.destroy', ['id' => $event->id]) }}" onsubmit = "return confirm('Are you sure?')">
                        <input type="hidden" name="_method" value="DELETE">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <a href="{{ route('file-package.edit', ['id' => $event->id]) }}" class="btn btn-warning  btn-margin">
							Update
                        </a>
