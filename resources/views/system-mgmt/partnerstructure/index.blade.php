@extends('system-mgmt.partnerstructure.base')
@section('action-content')
  ...
  <div class="box-header">
    <div class="row">
        ...
        <div class="col-sm-4">
          <a class="btn btn-primary" href="{{ route('partnerstructure.create') }}">Add new </a>
        </div>
    </div>
  </div>
  <!-- /.box-header -->
  <div class="box-body">
      ...
      <form method="POST" action="{{ route('partnerstructure.search') }}">
         {{ csrf_field() }}
          ...
          <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
            ...
            @foreach ($structures as $structure)
                <tr role="row" class="odd">
                  ...
                  <td>
                    <form class="row" method="POST" action="{{ route('partnerstructure.destroy', ['id' => $structure->id]) }}" onsubmit = "return confirm('Are you sure?')">
                        <input type="hidden" name="_method" value="DELETE">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <a href="{{ route('partnerstructure.edit', ['id' => $structure->id]) }}" class="btn btn-warning col-sm-3 col-xs-5 btn-margin">
                        Update
                        </a>
