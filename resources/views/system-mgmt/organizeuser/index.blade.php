...
@section('action-content')
  ...
  <div class="box-header">
    <div class="row">
        ...
        <div class="col-sm-4">
          <a class="btn btn-primary" href="{{ route('organizeuser.create') }}">Add new organize</a>
        </div>
    </div>
  </div>
  <!-- /.box-header -->
  <div class="box-body">
      ...
      <form method="POST" action="{{ route('organizeuser.search') }}">
         {{ csrf_field() }}
      ...
      <div class="row">
        <div class="col-sm-12">
          ...
          <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
            <thead>
              <tr>
				  <th   tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="organize: activate to sort column ascending">ลำดับที่</th>
            ...
            @foreach ($structures as $key => $structure)
                <tr role="row" class="odd">
                  ...
                  <td>
                    <form class="row" method="POST" action="{{ route('organizeuser.destroy', ['id' => $structure->id]) }}" onsubmit = "return confirm('Are you sure?')">
                        <input type="hidden" name="_method" value="DELETE">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <a href="{{ route('organizeuser.show', ['id' => $structure->id]) }}" class="btn btn-info   btn-margin">
                        View
                        </a>
                        <a href="{{ URL::to('/organizeuser/branch', ['id' => $structure->id]) }}" class="btn btn-success   btn-margin">
                        Branch
                        </a>
                        <a href="{{ route('organizeuser.edit', ['id' => $structure->id]) }}" class="btn  btn-warning  btn-margin">
                          Update
                        </a>
