    ...
    <div class="row">
        <div class="col-md-8 col-md-offset-2">
            <div class="panel panel-default">
                <div class="panel-heading">Update Userauth</div>
                <div class="panel-body">
                    <form class="form-horizontal" role="form" method="POST" action="{{ route('userauth.update', ['id' => $userauth->id]) }}">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-6">
                              ...
                              <option value="{{$user->id}}" {{$user->id == $userauth->user_id ? 'selected' : ''}}>{{$user->username}}</option>
                                @endforeach
                              </select>
                            </div>
                        </div>
                        ...
                        <div class="col-md-6">
                          <select class=" form-control department" name="structure_id">
                              ...
                              @foreach ($structures as $structure)
                                  <option value="{{$structure->id}}"{{$structure->id == $userauth->structure_id ? 'selected' : ''}}>{{$structure->name}}</option>
                              @endforeach
                          ...
                          <select class="col-md-6 form-control name " name="block_id" required autofocus>
                              ...
                              @foreach ($blocks as $block)
                                  <option value="{{$block->id}}"{{$block->id == $userauth->block_id ? 'selected' : ''}}>{{$block->name}}</option>
                              @endforeach
                          </select>
                        </div>
                        </div>
