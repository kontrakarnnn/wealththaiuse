@extends('system-mgmt.noti.sbase')
...
<div class="container" >
        ...
        <p><p style="display: inline; font-weight: bold;">Note : </p>{{ $notis->sender_note}}   </p>
          ...
          {{--}} <p><p style="display: inline; font-weight: bold;">Status : </p>{{ $notis->status}}   </p>--}}
           <form class="form-horizontal" role="form" method="POST" action="{{ route('noti.update', ['id' => $notis->id]) }}">
...
<form method="POST" action="{{ route('noti.destroy', ['id' => $notis->id]) }}" onsubmit = "return confirm('Are you sure?')">
    ...
    <a class="btn btn-default btn-margin" href="{{ URL::to('noti/reply',$notis->id)}}">
        Reply
      </a>

      <a class="btn btn-default btn-margin " href="{{ URL::to('noti/forward',$notis->id)}}">
          Forward
        </a>
