...
@foreach ($findoffer as $index =>$chunk)
        <table style='margin-top:160px' class='table'>
          ...
          <tr>
            <th class='table'style='background-color:#dedee3;text-align:left; '>ภาษีหัก ณ ที่จ่าย (รวม)</th>
            @foreach($chunk as $findof)
            @php
            $offeract5 = App\Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value5');
            $offertax5 = App\Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value5');
            $allresultpaytax = ($offeract5+$offertax5)+$findof->offer_payment_value5;
            @endphp
            <td class='table'>{{$allresultpaytax}}</td>
            @endforeach
          </tr>
          <tr>
            <th class='table'style='background-color:#dedee3;text-align:left; '>ค่าคอมมิชชั่นผู้ให้บริการ(รวม)</th>
            @foreach($chunk as $findof)
            @php
            $offeract19 = App\Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value19');
            $offertax19 = App\Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value19');
            $allresultucom = ($offeract19+$offertax19)+$findof->offer_payment_value19;
            @endphp
            <td class='table'>{{$allresultucom}}</td>
            @endforeach
          </tr>
          <tr>
            <th class='table'style='background-color:#dedee3;text-align:left; '>เบี้ยที่ผู้ให้บริการต้องชำระ(รวม)</th>
            @foreach($chunk as $findof)
            @php
            $conclu = $result-$allresultpaytax-$allresultucom;
            @endphp
            <td class='table'>{{$conclu}}</td>
            @endforeach
          </tr>
        </table>
