@extends('system-mgmt.event-regis-status.base')
@section('action-content')
  ...
  <div class="box-header">
    <div class="row">
        ...
        <div class="col-sm-8">
          <h3 class="box-title">List of Event Register Status</h3>
        </div>
        <div class="col-sm-4">
          <a class="btn btn-primary" href="{{ route('event-regis-status.create') }}">Add New</a>
        </div>
    </div>
  </div>
  <!-- /.box-header -->
  <div class="box-body">
	  ...
	  {{--
      <form method="POST" action="{{ route('event-regis-status.search') }}">
         {{ csrf_field() }}
          ...
          <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
            ...
            @foreach ($leadstatus as $status)
                <tr role="row" class="odd">
                    <td>{{ $status->name }}</td>
                    <td>
                    <form class="row" method="POST" action="{{ route('event-regis-status.destroy', ['id' => $status->id]) }}" onsubmit = "return confirm('Are you sure?')">
                        <input type="hidden" name="_method" value="DELETE">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <a href="{{ route('event-regis-status.edit', ['id' => $status->id]) }}" class="btn btn-warning  btn-margin">
							Update
                        </a>
