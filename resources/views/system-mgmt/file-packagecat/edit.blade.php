        ...
        <div class="col-md-8 col-md-offset-2">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <form class="form-horizontal" role="form" method="POST" action="{{ route('file-packagecat.update', ['id' => $event->id]) }}">
                        <input type="hidden" name="_method" value="PATCH">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">


                        <div class="form-group{{ $errors->has('structure_id') ? ' has-error' : '' }}">
                      <label class="col-md-4 control-label">File Package</label>
                        <div class="col-md-6">
                          <select class=" form-control department" name="package_id">

                              <option value="" >-Select-</option>
                              @foreach ($filepackage as $ser)
                                  <option value="{{$ser->id}}"{{$ser->id == $event->package_id ? 'selected' : ''}}>{{$ser->name}}</option>
                              @endforeach
