@extends('system-mgmt.asset.base2')
    ...
    <div id="example2_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
      <div class="row">
        <div class="col-sm-12">
          ...
          <th width="15%" class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="port: activate to sort column ascending">Referal Information</th>
          <th width="" class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="port: activate to sort column ascending">Issuer</th>
            ...
            <tbody>
              ...
              @foreach ($porttypes as $port)
                ...
                <tr role="row" class="odd">
                  <td>{{ $port->la_nla}}</td>
                  <td>{{ $port->asset_type_name }} {{ $port->asset_subtype_name }}</td>
                  <td>{{ $port->name }}</td>
					<td>{{ $port->ref_name }}</td>
         		 <td>{{ $port->ref_info1 }}</td>
         		 <td>{{ $port->issuer }}</td>
                  ...
                  <td>
                    <form class="row" method="POST" action="{{ route('asset.destroy', ['id' => $port->id]) }}" onsubmit = "return confirm('Are you sure?')">
                        ...
                        <a href="{{ URL::to('person/port/asset/detail',[$port->id,$r])}}" class="btn btn-info btn-margin">
                        Details
                        </a>
...
<th width="" class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="port: activate to sort column ascending">LA/NLA</th>
                ...
                <th width="" class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="port: activate to sort column ascending">Name</th>
				  <th width="" class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="port: activate to sort column ascending">Referal Name</th>
          <th width="" class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="port: activate to sort column ascending">Referal Information</th>
          <th width="" class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="port: activate to sort column ascending">Issuer</th>
                <th tabindex="0" aria-controls="example2" rowspan="1" colspan="2" aria-label="Action: activate to sort column ascending">Action</th>
              </tr>
            </tfoot>
