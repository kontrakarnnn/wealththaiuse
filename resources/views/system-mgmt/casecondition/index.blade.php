...
@section('action-content')
  ...
  <div class="box-header">
    <div class="row">
        <div class="col-sm-8">
          <h3 class="box-title">List of Case Condition</h3>
        </div>
        <div class="col-sm-4">
          <a class="btn btn-primary" href="{{ route('casecondition.create') }}">Add new Case Condition</a>
        </div>
    </div>
  </div>
  <!-- /.box-header -->
  <div class="box-body">
      ...
      <form method="POST" action="{{ route('casecondition.search') }}">
         {{ csrf_field() }}
    ...
    <div id="example2_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
      <div class="row">
        <div class="col-sm-12">
          <a  style="color:blue"href="/admin/casecondition" for="file_category_name" class="btn btn-default "> ShowAll </a>

			<div style="overflow-x:auto;">
          <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
            <thead>
              <tr role="row">
                <th >Case Condition Name</th>
                <th>Case Name</th>
                <th>Path Condition Detail</th>
            ...
            @foreach ($data as $da)
                <tr role="row" class="odd">
                  <td>{{$da->name}}</td>
                  <td>{{$da->Cases->name}}</td>
                  <td>{{$da->Path_condition_detail->name}}</td>
                  <td>{{$da->condition_flag}}</td>
                  <td>{{$da->description}}</td>
                  <td>
                    <form class="row" method="POST" action="{{ route('casecondition.destroy', ['id' =>$da->id]) }}" onsubmit = "return confirm('Are you sure?')">
                        <input type="hidden" name="_method" value="DELETE">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <a href="{{ route('casecondition.edit', ['id' => $da->id]) }}" class="btn btn-warning  btn-margin">
                        Update
                        </a>
