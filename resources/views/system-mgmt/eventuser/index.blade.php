...
@section('action-content')
  ...
  <div class="box-body">
      ...
      <form method="POST" action="{{ route('eventuser.search') }}">
         {{ csrf_field() }}
          ...
          <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
            ...
            @foreach ($events as $event)
                <tr role="row" class="odd">
                  ...
                  <td>
                    <form class="row" method="POST" action="{{ route('eventuser.destroy', ['id' => $event->id]) }}" onsubmit = "return confirm('Are you sure?')">
                        <input type="hidden" name="_method" value="DELETE">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <a href="{{ URL::to('/eventuser/eventregis',$event->id)}}" class="btn btn-info  btn-margin">
  Register Event
            </a>

                  <a href="{{ URL::to('/eventuser/checkevent',$event->id)}}" class="btn btn-primary  btn-margin">
More Details
</a>
                {{--}}        <a href="{{ route('eventuser.edit', ['id' => $event->id]) }}" class="btn btn-warning  btn-margin">
							Update
            </a>--}}
