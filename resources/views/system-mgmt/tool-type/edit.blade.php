        ...
        <div class="col-md-8 col-md-offset-2">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <form class="form-horizontal" role="form" method="POST" action="{{ route('tool-type.update', ['id' => $data->id]) }}">
                        <input type="hidden" name="_method" value="PATCH">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">

                        <div class="form-group{{ $errors->has('structure_id') ? ' has-error' : '' }}">
                      <label class="col-md-4 control-label">Tool Category</label>
                        <div class="col-md-6">
                          <select class=" form-control department" name="cat_id">

                              <option value="0" >-Select-</option>
                              @foreach ($toolcat as $tool)
                                  <option value="{{$tool->id}}"{{$tool->id == $data->cat_id ? 'selected' : ''}}>{{$tool->name}}</option>
                              @endforeach
                        ...
                        <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
                            <label for="name" class="col-md-4 control-label">Tool Type Name</label>
