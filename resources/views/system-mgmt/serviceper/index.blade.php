        ...
        <div class="col-md-6">
          <div class="form-group">
                ...
                <select class=" form-control department" name="structure_id" onchange="window.location.href=this.value;"required>
                                      ...
                                      @foreach ($group as $g)
                      <option    value="{{url('service/group')}}/{{$g->id}}">{{$g->name}}</option>
                  @endforeach
                    @foreach ($serg as $ser)
                        <option value="{{url('service/group')}}/{{$ser->id}}" >{{$ser->name}}</option>
                    @endforeach
              ...
              <label for="input" class=" control-label">Select Service Name</label>
                ...
                <select class="form-control name"size="1" id="nameid" name="links" onchange="window.location.href=this.value;">

                    @foreach ($serv as $s)
                      <option value="{{url('service')}}/{{$s->id}}">{{$s->name}}</option>
                  @endforeach
                </select>
                <br>
			                             <label href="/allservice">show all
                    <input class="checkbox "type="checkbox"  value="/allservice" name="checket"
...
<label>หมายเลขบัญชี (portfolio number)</label>
...
<input type="hidden"id="sender_id" type="text" class="form-control" name="citi" value=" {{$serviccente->citizen_id}}">
<div class="form-group">

  <div class="col-md-4">
    @if($serviccente->parallel == NULL)
    @if(App\Noti::where('message_type_id',$serviccente->msg_type_id)->latest()->where('sender_id',$matchids)->value('status') ==  "Request")
  ...
  @endif
@elseif ($serviccente->parallel == "Yes")
  <button type="submit" class="btn btn-outline-primary">
      Sent
  </button>
                ...
                success:function(data){
                 ...
                 console.log(data.length);
                  ...
                  for(var i=0; i<data.length;i++){
                    op+='<option value="{{url('service')}}/'+data[i].id+'">'+data[i].name+'</option>';
