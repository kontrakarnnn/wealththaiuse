          ...
          <div class="row">
              <div class="col-md-8 col-md-offset-2">
                  <div class="panel panel-default">
                      <div class="panel-heading">Referal Management</div>
                      <div class="panel-body">


                        <form class="" role="form" method="POST" action="{{ route('reflink.store') }}">
                            {{ csrf_field() }}
                                <div class="form-group">
                                <label >Select Event For Quick Register</label>



                                    <select onchange="myFunctions()" id="select-eventss" class="form-control department" name="event_id">

                                      <option value="" >-Select-</option>
                                    @foreach ($events as $event)
                                      <option value="{{$event->id}}"{{$event->id == $eventl ? 'selected' : ''}}>{{$event->event_name}}</option>
                                            @endforeach
                                    ...
                                    <div class="column">
                                      <div class="form-group">
                                        ...
                                        <button type ="submit"class="btn btn-warning btn-margin" >Generate Link</button>
										  <a href="/reflink" class="btn btn-danger btn-margin">Clear</a>
                                      </div>
                                  ...
                                  <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
                                      <label for="name" class="">Input Your Website</label>

                                      <div class="">
                                        <textarea id="link2"  name="website" placeholder="Input Your Website Here" class="form-control"  type="text"></textarea>
                                          <br>
                                      </div>
                                  </div>
                                ...
                                @if($website != NULL)
                                <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
                                    <label for="name" class="">SmartLink</label>

                                    <div class="">
                                      <textarea id="link3" readonly="readonly" placeholder="Input Your Website Here" class="form-control"  type="text" value="{{ url('/perregis/create') . '?ref?' . $currentmatchids . '?refevent?' . $eventl . '?UTM??utm_source=' . $source . '&utm_medium=' . $medium . '&utm_campaign=' . $name . '&utm_term=' . $term . '&utm_content=' . $content}}" >{{ $website  . '?ref?' . $currentmatchids . '?refevent?' . $eventl . '?UTM??utm_source=' . $source . '&utm_medium=' . $medium . '&utm_name=' . $name . '&utm_term=' . $term . '&utm_content=' . $content }}</textarea>
                                        <br>
                                      <button class="btn btn-info" onclick="myFunction3()">Copy Link</button>
                                    </div>
                                </div>
                                @else
                                @endif
                              <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
                                  <label for="name">Quick Register Referal</label>

                                  <div class="">
                                    <textarea id="link3"class="form-control" readonly="readonly" type="text" value="{{ url('/quickregister') . '?ref?' . $currentmatchids . '?refevent?' . $eventl . '?UTM??utm_source=' . $source . '&utm_medium=' . $medium . '&utm_name=' . $name . '&utm_term=' . $term . '&utm_content=' . $content }}">{{ url('/quickregister') . '?ref?' . $currentmatchids . '?refevent?' . $eventl . '?UTM??utm_source=' . $source. '&utm_content=' . $content . '&utm_medium=' . $medium . '&utm_name=' . $name . '&utm_term=' . $term  }}</textarea>
                              ...
                              <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
                                  ...
                                  <div class="">
                                    <textarea id="link2" class="form-control" readonly="readonly" type="text" value="{{ url('/perregis/create') . '?ref?' . $currentmatchids . '?refevent?' . $eventl . '?UTM??utm_source=' . $source . '&utm_medium=' . $medium . '&utm_campaign=' . $name . '&utm_term=' . $term . '&utm_content=' . $content}}" >{{ url('/perregis/create')  . '?ref?' . $currentmatchids . '?refevent?' . $eventl . '?UTM??utm_source=' . $source . '&utm_content=' . $content . '&utm_medium=' . $medium . '&utm_name=' . $name . '&utm_term=' . $term }}</textarea>
                                      <br>
                                    <button class="btn btn-info" onclick="myFunction2()">Copy Link</button>
                                                            ...
                                                            <div class="row">
                                <div class="col-sm-5">
                                  <div class="dataTables_info" id="example2_info" role="status" aria-live="polite"><b style="text-align:center;color:red">Lasts 10 Leads</b></div>
